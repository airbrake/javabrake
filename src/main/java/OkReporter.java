...
class OkReporter {
  ...
  OkReporter(int projectId, String projectKey) {
  ...
  String noticeJson(Notice notice) {
    ...
    for (int level = 0; level < 8; level++) {
      data = OkReporter.gson.toJson(notice);
      if (data.length() < OkReporter.maxNoticeSize) {
        break;
      }
  ...
  void parseResponse(Response resp, Notice notice) {
    ...
    NoticeIdURL data = OkReporter.gson.fromJson(resp.body().charStream(), NoticeIdURL.class);
    notice.id = data.id;
    notice.url = data.url;
  }
}
